...
Shared version: [Config](./config.md#rulecommonsettings). Catalog: [Renames and splits](./rule-names.md).
...
In v4, `wai-aria` ran many checks, some gated by boolean options (`packages/@markuplint/rules/src/wai-aria/index.ts` at `v4.18.3`).
...
`landmark-roles` and `required-h1` splits are in [Renames and splits](./rule-names.md), not in the `wai-aria` alias.
