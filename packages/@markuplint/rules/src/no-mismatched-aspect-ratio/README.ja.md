...
- 比較には ±0.5 ピクセルの許容誤差があります。`width`/`height` 属性は整数のため、実際の比率が割り切れない画像は整数値と厳密に一致することがありません。許容誤差の計算には交差積(`|attrWidth * actualHeight - attrHeight * actualWidth| / actualWidth`)を使用し、浮動小数点の誤差を回避しつつピクセル相当の単位で不一致を表現しています。
