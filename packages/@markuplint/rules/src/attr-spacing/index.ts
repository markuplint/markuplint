...
export interface AttrSpacingOptions {
	lineBreak: 'either' | 'always' | 'never';
	width: number | false;
}

const verifyWalker: ElementVerifyWalkerFactory<boolean, AttrSpacingOptions> = (reports, translate) => node => {
...
const fixWalker: ElementFixWalker<boolean, AttrSpacingOptions> = node => {
	const attrs = node.attributes;
	for (const attr of attrs) {
		if (attr.attrType === 'ps-attr') {
			continue;
		...
		if (!hasSpace) {
			...
			if (hasLineBreak) {
				...
				} else if (node.rule.option.width && node.rule.option.width !== attr.spacesBeforeName.raw.length) {
					attr.spacesBeforeName.fix(expectSpaces);
				}
			}
		}
...
export default createRule<boolean, AttrSpacingOptions>({
	name: 'attr-spacing',
