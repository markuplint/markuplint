...
When `min` is omitted it defaults to `0`; when `max` is omitted it defaults to `1`; when `value` is omitted it defaults to `min`. Authored-but-unparsable attribute values are deferred to [`no-invalid-attr-value`](../no-invalid-attr-value/) and skipped here to avoid compounding diagnostics.
