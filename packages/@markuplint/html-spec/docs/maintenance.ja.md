...
`index.json` を書き出す前に、`main()` は生成された spec を検証し（`generator/validate.ts`）、スクレイピング失敗によるデータ損失を検出します。いずれかのチェックに失敗すると、問題を一覧表示して生成を中止し、`index.json` は変更されません。

| チェック           | 要件                                       | 失敗が示す典型的な原因                             |
| ------------------ | ------------------------------------------ | -------------------------------------------------- |
| 空の説明           | HTML要素の50%以下                          | MDN の取得失敗、スクレイピングセレクタの破損       |
| コア要素の存在     | `html`、`body`、`div`、`a` …（リスト参照） | 要素リスト自体の読み込み失敗                       |
| 参照URL（`cites`） | 最低1件                                    | 取得の全面失敗・ネットワーク不通                   |
| 各版のARIAロール   | 非空、`button` と `link` を含む            | ARIA 仕様スクレイプが空データを返した              |
| 要素数の安定性     | 直前にコミットされた件数の85%以上          | スクレイピング／パースのリグレッションで要素が欠落 |

要素数チェックは、上書き前に読み込んだ直前のコミット済み `index.json` と比較します（初回生成時はスキップ）。閾値と必須名リストは `generator/validate.ts` 冒頭の定数（`ELEMENT_COUNT_STABILITY_THRESHOLD`、`REQUIRED_ELEMENTS`、`REQUIRED_ARIA_ROLES`）です。正当な仕様変更でチェックに引っかかる場合はここで調整します。

生成成功後、`main()` は `generator-summary.md`（gitignore 済みの実行ごとの生成物）も書き出します。これは要素の追加・削除、各版の ARIA ロール／プロパティ変更、参照URL数の Markdown 差分を含みます。**Update HTML Spec** ワークフローはこれを自動更新 PR の本文に入れ、レビュアーが生成された `index.json` を diff せずに変更内容を把握できるようにします。
