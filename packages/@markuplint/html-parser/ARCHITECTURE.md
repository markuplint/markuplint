...
| Method              | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tokenize()`        | Invokes parse5 `parse()` or `parseFragment()` based on fragment detection, **wiring `onParseError` to collect non-fatal parser-conformance events into `MLASTDocument.parseErrors`** (consumed by `@markuplint/ml-core` as `ruleId: 'parse-error'` violations). The empty-span `raw` field is filled via `extractRawForParseError()` so the violation excerpt shows the surrounding token text (attribute name, character reference, etc.). |
