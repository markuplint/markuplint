...
- [概要](#概要)
- [上書きセマンティクス（設計契約）](#上書きセマンティクス設計契約)
...
このドキュメントで説明するマージは、**サイレントかつ無条件に上書きする加算的オーバーレイ**です。これは意図的な設計判断です（[#3893](https://github.com/markuplint/markuplint/issues/3893) 参照）:

- **後の仕様が勝ちます。** `ExtendedSpec` の各セクションはスプレッドまたは名前ベースのマージで結果に畳み込まれ、同じキー・名前についてはエクステンションの宣言がベース定義を置き換えます。
- **競合検出は存在しません。** エクステンションがベースの HTML 仕様と矛盾していないかを検査するバリデーションはなく、上書きが発生してもログは出ません。HTML Living Standard が禁止している定義でも、フレームワーク仕様はサイレントに許可できます。
- **これはバグではなく機能です。** フレームワーク仕様は、フレームワークが有効にする構文のために HTML の制約を緩和・拡張することがまさに目的です。例えば `@markuplint/react-spec` は JSX 専用属性の `dangerouslySetInnerHTML` を追加し、`contenteditable` の enum に `'inherit'` を加えます。`@markuplint/vue-spec` は `v-if` / `v-for` などのディレクティブをグローバル属性として追加します。
- **責任は `ExtendedSpec` の作者にあります。** 上書きはサイレントであるため、意図せずベースの制約を弱めてもどこにも警告は出ません。`ExtendedSpec` を作成・レビューする際は、ベース仕様とのキー衝突をすべて意図的な判断として扱ってください。
- **実行時に出所は記録されません。** マージ済みの `MLMLSpec` は、どの仕様が定義を提供したかを保持しません。フレームワーク仕様を有効化した後に違反が消えた場合は、エクステンションのソースをベース仕様と diff して上書き箇所を特定してください。
