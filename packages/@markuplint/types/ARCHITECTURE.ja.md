...
│   ├── check-url.ts                # 妥当な URL 文字列の検証 — `new URL()` が暗黙的に自動補正してしまう URL Living Standard の validation error を捕捉。全カテゴリと属性ごとの URL バリアント (`URL` / `NonEmptyURL` / `BaseURL` / `AbsoluteURL` / `AbsoluteURLOrEmpty`) の一覧は `docs/validators.md` の「URL Living Standard Validator」セクションを参照。
