...
This guard is dormant for parsers whose upstream tokenizer already rejects bare `<` in element body (`jsx-parser` via TypeScript ESTree, `mdx-parser` via remark-mdx) but is critical for parsers that hand the full raw element to `parseCodeFragment` (e.g., `astro-parser`).
