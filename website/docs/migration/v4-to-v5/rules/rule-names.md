...
This is the master reference for every rule-name change in v5. Search this page for the rule names in your current config to find what each becomes.

v5.0.0 redoes the rule catalog end to end: consistent verb-prefix naming, one rule per independent spec requirement, and machine-readable spec-conformance metadata. rc.4's 82 rules become 106 (25 splits, 1 new rule, 2 removed).
...
See also [ARIA Changes](/docs/migration/v4-to-v5/aria) for ARIA-specific detail, and the per-rule pages in this section for rules with option-format changes beyond the rename or split itself.

## Categories

rc.4's 5-category scheme (`validation`, `a11y`, `naming-convention`, `maintainability`, `style`) is replaced by 9 categories. `validation` was too coarse to browse — it held 53 of rc.4's 82 rules — so it is split by what the rule actually checks; `naming-convention` (1 rule, `class-naming`) folds into `style`.
...
Each split's checks correspond to independent spec requirements, so they can be enabled, disabled, or set to a different severity independently.
...
`aria-*` and `role` are exempt from all three spec-checking rules above — `no-unknown-attr`, `no-disallowed-attr`, and `no-invalid-attr-value` all skip them. The ARIA rules own them instead, `no-aria-on-unsupported-element` in particular.

The `wai-aria` umbrella rule's removal is a 21-way "split," if you count it that way — 20 of its 21 checks already had an independent successor before v5.0.0 shipped, and the 21st is the new `no-aria-on-unsupported-element`. See [ARIA Changes](/docs/migration/v4-to-v5/aria#umbrella-rule-removed).
...
:::note
The alias mechanism performs these expansions automatically, with a deprecation warning. The rest of this section only matters if you are rewriting your config by hand instead of relying on that.
:::
...
`no-experimental-features` and `no-nonstandard-features` are each omitted entirely unless the old config explicitly set the matching `check*` option to `true`. v4's default for both was `false` — that is, the check did not run at all.
...
| `input-button-non-empty-value` | `require-accessible-name` — see [ARIA Changes](/docs/migration/v4-to-v5/aria#removed-input-button-non-empty-value)                                                      |
...
- **`code-styles`** and **`security`** were empty in rc.4. `code-styles` now includes `case-sensitive-attr-name` and `case-sensitive-tag-name`; `security` now includes `no-event-handler-attr`.
...
- **`html-standard`** gains `itemprop-requires-itemscope` and drops `no-duplicate-dt` (a SHOULD) and `no-ineffective-attr` (non-normative) — the preset only admits rules whose spec conformance is `sources: ['html']` plus `level: 'must'`. `input-button-non-empty-value` is also gone from it, since the rule itself is removed.
