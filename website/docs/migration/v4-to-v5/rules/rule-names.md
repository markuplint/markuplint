---
sidebar_position: 0
title: Renames and Splits
---

# Rule Renames and Splits

This is the master reference for every rule-name change with a real v4 lineage. See [ARIA Changes](/docs/migration/v4-to-v5/aria) for the `wai-aria` umbrella rule (a special case — its split happened gradually across v5's pre-release development, not in one step), [New in v5](#new-in-v5-no-v4-equivalent) for rules that have no v4 predecessor at all, and the per-rule pages in this section for rules with option-format changes beyond the rename or split itself.

v5.0.0 redoes the rule catalog end to end: consistent verb-prefix naming, one rule per independent spec requirement, and machine-readable spec-conformance metadata. **v4's 38 rules become 106 in v5.0.0.** Of that growth: 12 rules were renamed outright, 10 rules were split into several (invalid-attr, doctype, character-reference, deprecated-attr, deprecated-element, landmark-roles, no-refer-to-non-existent-id, permitted-contents, required-h1, table-row-column-alignment), the `wai-aria` umbrella became 21 independent rules (see [ARIA Changes](/docs/migration/v4-to-v5/aria#umbrella-rule-removed)), 1 rule was removed outright (see [Deletions](#deletions)), and the rest are either entirely new capabilities with no v4 predecessor, or rules that already went through one rename during v5's alpha/rc development and are getting a second one now (see [New in v5](#new-in-v5-no-v4-equivalent)).
...
:::tip Nothing breaks silently — with one documented exception
Every renamed or split rule keeps working under its old name. Markuplint reports a deprecation warning and expands the old config to the new rule(s) automatically. Old names are removed in v6.

The exception: two rules keep their pre-v5 name and gain a split-off sibling with no alias to announce it. See [Known migration gap](#known-migration-gap) if you use `permitted-contents` or `no-refer-to-non-existent-id` directly in a raw (non-preset) config.
:::

## Categories

v4's 5-category scheme (`validation`, `a11y`, `naming-convention`, `maintainability`, `style`) is replaced by 9 categories, each rule's `meta.ts` `category` field. `validation` was too coarse to browse — it held the largest share of v4's rules — so it is split by what the rule actually checks; `naming-convention` (1 rule, `class-naming`) folds into `style`.
...
Categories are a browsing aid on the [rules index](/docs/rules); they never appear in a config file, so there is nothing to migrate here.
...
|                               | `no-disallowed-ancestor`           | Appears as a descendant of a forbidden ancestor (the spec's `forbiddenAncestors`)                                                                         |
|                               | `require-ancestor`                 | Missing a required ancestor (the spec's `descendantOf`)                                                                                                   |
...
|                               | `consistent-table-row-length`      | Inconsistent column count across rows (the spec permits this; the only `warning`-level entry of the four)                                                 |

:::note Two more splits have no v4 predecessor
`wai-aria-disallowed-props` and `wai-aria-implicit-props` — split respectively into `no-prohibited-naming`/`element-supports-aria-prop`/`role-supports-aria-prop` and `no-redundant-aria-prop`/`no-contradictory-aria-prop` — look like splits of a v4 rule, but neither name existed in a real v4 release. Both were themselves pieces of the `wai-aria` umbrella's earlier pre-release split. See [ARIA Changes](/docs/migration/v4-to-v5/aria#umbrella-rule-removed) for the full `wai-aria` → 21-rules story that covers them, and `no-unsupported-features`, `script-content`, and `srcset-sizes-constraint` in [New in v5](#new-in-v5-no-v4-equivalent) for three more "looks like a split, has no v4 predecessor" cases.
:::

### Option-routed splits: before / after

Most splits above are unconditional — every old check gets its own new rule regardless of options. Five are not: `doctype`, `landmark-roles`, `required-h1`, `no-unsupported-features`, and `invalid-attr` route to a subset of their new siblings depending on what the old options said.

:::note
The alias mechanism performs these expansions automatically, with a deprecation warning. The rest of this section only matters if you are rewriting your config by hand instead of relying on that. (`no-unsupported-features` is the [New in v5](#new-in-v5-no-v4-equivalent) rule with no v4 predecessor — this walkthrough is for anyone who already adopted it during v5's pre-release period.)
:::
...
`no-experimental-features` and `no-nonstandard-features` are each omitted entirely unless the old config explicitly set the matching `check*` option to `true`. The default for both was `false` — that is, the check did not run at all unless a config asked for it.

`invalid-attr` always expands to `no-unknown-attr`, `no-disallowed-attr`, and `no-invalid-attr-value`. `no-restricted-attr` joins them only when the old config actually set `disallowAttrs`, so a bare `invalid-attr: true` never enables a rule with nothing to restrict. The old options are routed rather than copied wholesale — see [`invalid-attr` rule changes](/docs/migration/v4-to-v5/rules/invalid-attr) for which option lands on which new rule.
...
| `no-unsupported-features`      | `no-unsupported-browser-features`, `no-experimental-features`, `no-nonstandard-features`                                   | browserslist × BCD unsupported features, experimental-flagged features, non-standard-flagged features (see [above](#option-routed-splits-before--after) for how the three relate) |
...
| ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `wai-aria` (umbrella)                                                   | Its 21 successor rules: 20 already independent, plus the new `no-aria-on-unsupported-element` — see [ARIA Changes](/docs/migration/v4-to-v5/aria#umbrella-rule-removed) |
| `input-button-non-empty-value` (v5 pre-release only, no v4 predecessor) | `require-accessible-name` — see [ARIA Changes](/docs/migration/v4-to-v5/aria#removed-input-button-non-empty-value)                                                      |
...
- **`label-has-control`**: detects only a `<label>` with no associated control now. Detecting a _second_ control inside the same `<label>` was already `label-no-multiple-controls`'s job; `label-has-control` reporting it too was a duplicate.

## Known migration gap {#known-migration-gap}

:::danger No deprecation warning for these two
Two rules keep their name — not renamed, not deprecated — but gain a sibling from a split: `permitted-contents` (→ `no-disallowed-ancestor` / `require-ancestor` / `no-duplicate-sibling-attr`) and `no-refer-to-non-existent-id` (→ `no-broken-fragment-link`).
...
to keep the same coverage in v5. Likewise, `no-refer-to-non-existent-id` needs `no-broken-fragment-link` added alongside it.
...
| `no-obsolete-attr` / `no-obsolete-element`                                  | `error` (as `deprecated-attr` / `deprecated-element`) | `error` (unchanged) | Obsolete (spec-removed) is a MUST                                                                                                                         |
| `no-deprecated-attr` / `no-deprecated-element`                              | `error` (as `deprecated-attr` / `deprecated-element`) | `warning`           | Factual, MDN/BCD-sourced data, not a spec MUST                                                                                                            |
...
`no-consecutive-br` is a deliberate exception: it stays `warning` even though it is a proxy for an HTML LS MUST, because the detection can false-positive on legitimate uses such as poem line breaks.

:::caution This can turn a green pipeline red
The `warning` → `error` row above (table model errors) covers three rules. Any of them can fail CI on code you did not touch, if your team uses a strict zero-warnings gate such as `--max-warnings 0`. Check your current warning counts against these specific rules before upgrading — and against the three new `error`-level checks noted above, if you already adopted them during v5's pre-release period.
:::

## Preset changes

- **`code-styles`** and **`security`** were empty in v4 (verified: both presets were still `{}` in the last stable v4 release). `code-styles` now includes `case-sensitive-attr-name` and `case-sensitive-tag-name`; `security` now includes `no-event-handler-attr`.
- **`performance`** gains `head-element-order` and `no-mismatched-aspect-ratio` as plain rules. Its existing `nodeRules`-scoped entries are unchanged. Neither has a v4 predecessor — see [New in v5](#new-in-v5-no-v4-equivalent) for `no-mismatched-aspect-ratio`'s pre-release history; `head-element-order` is new outright.
- **`html-standard`** drops `no-duplicate-dt` (a SHOULD) and `no-ineffective-attr` (non-normative) — both were present in v4's `html-standard` preset, so this is a genuine removal; the preset only admits rules whose spec conformance is `sources: ['html']` plus `level: 'must'`. `input-button-non-empty-value` is also gone from it, since the rule itself is removed. It also gains `itemprop-requires-itemscope`, which has no v4 predecessor at all.
