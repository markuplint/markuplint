...
### Additional patterns not covered above

More areas the same rule family tightened during the RC cycle, not captured in the table above. Same rules, same "no config action required" status.

| Area                                                                             | Example that now fails                                                                          | Issue                                                         | Spec                                                                                                                          |
| -------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `script` attribute applicability table                                           | `<script type="module" defer src="a.js">`, `<script blocking="render">` (no `src`)              | —                                                             | [HTML LS §4.12.1](https://html.spec.whatwg.org/multipage/scripting.html#attr-script-defer)                                    |
| `meta[http-equiv=content-type]` requires literal `charset=utf-8`                 | `<meta http-equiv="Content-Type" content="text/html; charset=Shift_JIS">`                       | —                                                             | [HTML LS — encoding declaration state](https://html.spec.whatwg.org/multipage/parsing.html#attr-meta-http-equiv-content-type) |
| `meta[http-equiv=content-security-policy]` CSP3 grammar                          | `<meta http-equiv="Content-Security-Policy" content="not a policy">`                            | [#3942](https://github.com/markuplint/markuplint/issues/3942) | [CSP Level 3](https://www.w3.org/TR/CSP3/)                                                                                    |
| `bdo[dir]` excludes `auto`, and is now required                                  | `<bdo dir="auto">`, `<bdo>` (no `dir`)                                                          | —                                                             | [HTML LS — the `bdo` element](https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-bdo-element)               |
| `usemap="#"` (empty fragment)                                                    | `<img usemap="#">`                                                                              | [#3945](https://github.com/markuplint/markuplint/issues/3945) | [HTML LS — `HashName` reference](https://html.spec.whatwg.org/multipage/urls-and-fetching.html#hash-name-reference)           |
| `itemid`/`itemtype` require `itemscope`                                          | `<div itemid="https://example.com/item">` (no `itemscope`)                                      | [#3733](https://github.com/markuplint/markuplint/issues/3733) | [HTML LS §5.7.4](https://html.spec.whatwg.org/multipage/microdata.html#attr-itemid)                                           |
| `input` `min`/`max` value must match the type's format, `progress[max]` non-zero | `<input type="date" min="not-a-date">`, `<input type="number" min="abc">`, `<progress max="0">` | —                                                             | [HTML LS §4.10.5](https://html.spec.whatwg.org/multipage/input.html#the-input-element)                                        |
| `source[sizes]` requires `srcset`                                                | `<source sizes="100vw">` (no `srcset`)                                                          | —                                                             | [HTML LS — `source`](https://html.spec.whatwg.org/multipage/embedded-content.html#attr-source-sizes)                          |
