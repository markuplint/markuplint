...
- **`<link rel="alternate stylesheet">` requires `title`** (HTML LS §4.6.7.4). When `rel` contains both `alternate` and `stylesheet`, the spec mandates a `title` attribute with a non-empty value. The `required-attr` rule now fires when it is missing.
