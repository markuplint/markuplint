...
| [parse-error](/docs/migration/v4-to-v5/rules/parse-error) (built-in)    | The built-in channel can now surface non-fatal HTML LS tokenizer / tree-construction parse errors (parse5 `onParseError` events) in addition to fatal `ParserError`s. **Off by default** — opt-in per parse5 code via `severity.parseError`. `severity.parseError` also accepts a `Partial<Record<MLASTParseErrorCode, …>>` for per-code control. | Users who want to lint non-fatal HTML LS parse errors |
