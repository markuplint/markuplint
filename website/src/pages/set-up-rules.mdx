...
Create a file named `.markuplintrc` to the project directory that there is `package.json`. And add some rules to the `rules` property of this file.
...
If you want the part of structures only to apply some rule then set the tag name to `nodeRules` or set the selector to `childNodeRules`.
