...
Set a regular expression that can identify the target file name to the key, and the spec module path to the value.
...
| Nth-child pseudo-class                           | `:nth-child(2)` `:nth-child(2 of div)` `:nth-last-child(2)` `:nth-last-child(2 of div)` `:first-child` `:last-child` `:only-child` | ❌      |
