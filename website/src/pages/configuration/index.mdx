...
- [`nodeRules`](#properties/node-rules)
- [`childNodeRules`](#properties/child-node-rules)
...
[`specs`](#properties/specs),
[`specs`](#properties/specs),
and [`excludeFiles`](#properties/exclude-files) can set paths.
In `extends`, `plugins`, `parser,` and `specs` four, it can set a package name instead of a path.
...
If you set other config file [paths](#properties/specification-about-paths), it merges the current setting with them.
...
Set a package name or a [path](#properties/specification-about-paths).
Can set settings if the plugin has it.
...
Set a regex to the key, and the parser file [path](#properties/specification-about-paths) or a package name to the value.
The regex should be set it matches the target lintee file (ex., the extension part).
...
Set a regex to the key, and the spec file [path](#properties/specification-about-paths) or a package name to the value.
The regex should be set it matches the target lintee file (ex., the extension part).
...
Set a value for each rule.
Those are any strings, any number, and an array.
The rule becomes disabled if set false.
It evaluates as the default value if set true.
Otherwise, you can set details if you set structure like:
...
You can set the `rules` property of this property like the [`rules`](#properties/rules) property of the root.
...
You should set only element name to the `tagName`, but you can set `"#text"` exceptionally when you expect the _text node_.

### `childNodeRules`

If you want any specific element or its descendants to apply some rule, you can set by this property.
If set true to the `inheritance` property, _affects all descendant nodes_ of the target element.
