...
| [table-row-column-alignment](/docs/migration/v4-to-v5/rules/table-row-column-alignment) | テーブルを HTML LS の _forming a table_ アルゴリズムでモデル化。セルがひとつも開始しない列と行、行グループの末尾を越える `rowspan` を新たに報告。`rowspan` が下の行をちょうど埋めている場合に余分な列を報告しなくなった。                                                                                                                                                                                                                                                                                                                                                                                                                   | `colspan` / `rowspan` / `<col>` を使うテーブルを lint している人 |
