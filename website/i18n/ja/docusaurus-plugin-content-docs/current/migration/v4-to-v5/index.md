...
| [parse-error](/docs/migration/v4-to-v5/rules/parse-error) (組み込み)    | 組み込みチャネルが、致命的 `ParserError` に加えて HTML LS の非致命的なパースエラー (parse5 `onParseError` イベント) も surface できるように。**デフォルトはオフ** — `severity.parseError` で parse5 の code 単位にオプトイン。`severity.parseError` は `Partial<Record<MLASTParseErrorCode, …>>` 形式も受け付け、code ごとに severity を指定可能。 | 非致命的な HTML LS パースエラーも lint したいユーザー |
