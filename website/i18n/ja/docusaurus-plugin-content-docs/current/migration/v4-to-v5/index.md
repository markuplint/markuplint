...
| [設定](/docs/migration/v4-to-v5/config)              | 共通 ARIA バージョン用の `ruleCommonSettings` 追加。Named nodeRules による個別設定可能なチェック。配列値が連結から上書きに変更。Options が shallow merge に変更。               | 設定ファイル作成者、プリセット作成者 |
| [ARIA](/docs/migration/v4-to-v5/aria)                | ARIA 1.3 サポート追加（オプトイン）。`generic` ロールが透過的に。`image`/`img` ロールが同義語に。`wai-aria` オプションのリネーム。                                              | ARIA ルール利用者                    |
| [フレームワーク](/docs/migration/v4-to-v5/framework) | `@markuplint/htmx-parser` 削除（`@markuplint/htmx-spec` に移行）。`@markuplint/alpine-parser/spec` 削除（`@markuplint/alpine-spec` に移行）。`directivePatterns` システム追加。 | htmx / Alpine.js ユーザー            |
