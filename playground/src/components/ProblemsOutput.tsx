...
export const ProblemsOutput = ({ violations }: Readonly<{ violations: Violations | null }>) => {
	return (
		<section className="overflow-y-auto h-full text-sm">
			<h2 className="flex items-center gap-2 px-3 py-2 bg-slate-50 font-bold text-lg">
				Problems
				{violations && <span className="rounded-full bg-slate-200 px-2 py-1 text-sm">{violations.length}</span>}
			</h2>
			{violations === null ? (
