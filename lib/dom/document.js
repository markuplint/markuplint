                ...
                if (node instanceof text_node_1.default) {
                    ...
                    if (!(prevSyntaxicalTextNode instanceof raw_text_1.default)) {
                        ...
                        if (matched) {
                            ...
                            node.prevNode instanceof node_1.default &&
                        ...
                        for (const ruleName in nodeRule.rules) {
                            if (nodeRule.rules.hasOwnProperty(ruleName)) {
                                ...
                                if (nodeRule.tagName || nodeRule.selector) {
                                    ...
                                    else if (nodeRule.selector &&
                                        node instanceof element_1.default) {
    ...
    toDebugMap() {
        return this.list.map(n => {
            if (n instanceof node_1.default) {
                return `[${n.line}:${n.col}]>[${n.location.endLine}:${n.location.endCol}](${n.location.startOffset},${n.location.endOffset})${n instanceof omitted_element_1.default ? '???' : ''}${n.nodeName}: ${n
                    .toString()
                    .replace(/\n/g, '⏎')
                    .replace(/\t/g, '→')
                    .replace(/\s/g, '␣')}`;
