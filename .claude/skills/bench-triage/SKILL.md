...
| `An element with the attribute "role=alert" must not appear as a descendant of an "h1", "h2", "h3", "h4", "h5", or "h6" element.` (and the 13 sibling roles alertdialog/application/dialog/document/listbox/log/marquee/math/note/status/tabpanel/timer/toolbar) | **nu correct** — coverage gap tracked in #4028, not yet implemented | [ARIA in HTML "Allowed descendants of ARIA roles"](https://w3c.github.io/html-aria/#allowed-descendants) (informative): "Column 2 identifies the Kinds of content categories each `role` has when it is used on an HTML element" — these 14 roles are listed as Flow content. [HTML LS §4.3.6](https://html.spec.whatwg.org/multipage/sections.html#the-h1,-h2,-h3,-h4,-h5,-and-h6-elements) restricts `h1`–`h6` content to phrasing content only, so a role-bearing element in this set cannot be a heading descendant even when its underlying HTML element is itself phrasing content. `permitted-contents` has no role-driven content-category override yet. New to the nu-validator corpus as of submodule commit `f84563f` (2026-08-30); 14 fixtures under `html-aria/misc/role-*-inside-h1-novalid.html`. |
