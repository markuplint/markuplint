...
impl Rule for NoReferToNonExistentId {
    ...
    fn verify(&self, arena: &DomArena, spec: &MLMLSpec, config: &RuleConfigSet) -> Vec<Violation> {
        ...
        // Built from the spec rather than hardcoded so new ARIA versions are picked up.
        ...
        // A dynamic ID can't be verified statically, so skip the whole document.
        if has_dynamic_id {
            return violations;
        }
        ...
        for (node_id, el) in arena.elements() {
            ...
            for attr in &el.attributes {
                ...
                // Spec attribute types that denote an ID reference:
                // - `type === "DOMID"` → single ID reference
                // - `type.token === "DOMID"` with a separator → ID reference list
