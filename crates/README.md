...
- **Lint engine**: `Rule` trait (with `default_severity()` for per-rule severity defaults matching TS `createRule({ defaultSeverity })`), `lint()` function (MLAST + config + spec → violations), config parsing with `reason` field propagation through nodeRules/childNodeRules
