...
#[test]
#[ignore = "whitespace after doctype not yet inserted into implicit body"]
...
fn full_html_document() {
    ...
    assert!(maps.iter().any(|m| m.contains("head: <head>")), "head missing: {maps:?}");
    assert!(maps.iter().any(|m| m.contains("meta: <meta")), "meta missing: {maps:?}");
    assert!(maps.iter().any(|m| m.contains("head: </head>")), "/head missing: {maps:?}");
    assert!(maps.iter().any(|m| m.contains("body: <body>")), "body missing: {maps:?}");
    assert!(maps.iter().any(|m| m.contains("body: </body>")), "/body missing: {maps:?}");
    assert!(maps.iter().any(|m| m.contains("html: </html>")), "/html missing: {maps:?}");
}
...
#[test]
#[ignore = "invalid node handling not yet implemented"]
fn html_body_close_invalid() {
    // <html></body> should produce invalid node for </body>
    let _maps = debug_maps("<html></body>");
}

#[test]
#[ignore = "invalid end tag as ghost p/br not yet implemented"]
fn div_invalid_end_tags() {
    // <div></p></br></span></div> — ghost p, ghost br, invalid nodes
    let _maps = debug_maps("<div></p></br></span></div>");
}

#[test]
#[ignore = "offset options not yet implemented"]
fn offset_option() {
    let _maps = debug_maps("<span>\n\t\t\t<img src=\"path/to\">\n\t\t</span>\n\t\t\t");
}

#[test]
#[ignore = "frontmatter not yet implemented"]
fn with_frontmatter() {
    let _maps = debug_maps("---\np: v\n---\n<html></html>");
}

#[test]
#[ignore = "CRLF normalization not yet implemented"]
fn crlf_standard() {
...
#[test]
#[ignore = "form-in-form invalid handling not yet implemented"]
fn form_in_form() {
    let _maps = debug_maps("\n\t<form>\n\t\t<form novalidate>\n\t\t\t<input type=\"text\">\n\t\t</form>\n\t</form>\n\t");
}

#[test]
#[ignore = "noscript Issue #219 not yet implemented"]
fn noscript_issue_219() {
    let _maps = debug_maps("<html><body><noscript data-xxx><iframe ></iframe></noscript></body></html>");
}

#[test]
#[ignore = "noscript CRLF Issue #737 not yet implemented"]
fn noscript_issue_737() {
    let _maps = debug_maps("<html><body><noscript>\r\n<div>text</div>\r\n</noscript></body></html>");
}

#[test]
#[ignore = "standard large document test — needs full conformance"]
fn standard_large_document() {
    // The massive TS "standard code" test with comments, tables, templates, etc.
    let _maps = debug_maps("\n\t<!DOCTYPE html>\n\t<html lang=\"en\">\n\t<head>\n\t</head>\n\t<body>\n\t</body>\n\t</html>\n\t");
}
